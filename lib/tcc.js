  ...
  return queue.add(async () => {
    try {
      ...
      if (this.thermostats.LocationInfo && current.LocationInfo) {
        debug("pollThermostat - delta", JSON.stringify(diff.diff(this.thermostats, current), null, 2));
      }
  ...
  return new Promise((resolve, reject) => {
    (async () => {
      try {
        ...
        var message = '<?xml version="1.0" encoding="utf-8"?>' + parser.toXml(tccMessage.soapMessage(tccMessage.ChangeThermostatMessage(this.sessionID, desiredState, this.thermostats.hb[desiredState.ThermostatID])));
        ...
        debug("_UpdateThermostat message", parser.toXml(tccMessage.soapMessage(tccMessage.ChangeThermostatMessage(this.sessionID, desiredState, this.thermostats.hb[desiredState.ThermostatID]))));
        reject(err);
        this.sessionID = null;
      }
    })();
