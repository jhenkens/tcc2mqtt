...
function MqttThermostat(wrapper, that, device) {
  ...
  this.unit = that.config['temperature_unit'];
  ...
  this.away_temperature = that.config['away_temperature'] !== undefined
    ? new Temperature(that.config['away_temperature'], this.unit)
    : undefined;
  this.away_mode = that.config['away_heat_cool_mode'];
...
MqttThermostat.prototype.setUnavailable = async function(message){
  ...
  await this.mqtt.publish(this.status_topic, "unavailable", {retain: true});
  await this.mqtt.publish(this.status_json_attributes_topic, JSON.stringify(this.status_data), {retain: true});
}
