...
const publishOptimisticChangeQueue = new PQueue({
    concurrency: 1
});
...
function MqttThermostat(wrapper, config, device) {
    ...
    this.update_state_optimistically = config.update_state_optimistically
        ...
        case this.mode_command_topic:
            if (this.use_auto_for_schedule_in_homekit) {
                if (message === "auto") {
                    ...
                    this.publishOptimisticChange(this.mode_state_topic, message)
                    break;
...
MqttThermostat.prototype.setTemp = function (target, permanent = undefined) {
    ...
    this.publishOptimisticChange(this.target_temp_state_topic, target);
};
...
MqttThermostat.prototype.setHold = function (target, lowPriority = false) {
    ...
    this.publishOptimisticChange(this.hold_state_topic, target);
};
...
MqttThermostat.prototype.setMode = function (target, permanent = undefined) {
    ...
    this.publishOptimisticChange(this.mode_state_topic, target);
};
...
MqttThermostat.prototype.publishOptimisticChange = function (
    key, value
) {
    if(!this.update_state_optimistically) {
        return;
    }
    const newState = {}
    newState[key] = value;
    publishOptimisticChangeQueue.add(() => this.publishStateAsync(newState, true));
};
