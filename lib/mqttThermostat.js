...
MqttThermostat.prototype.updateStatusAsync = async function(device, force = false){
  ...
  this.setAvailability(alerts === undefined);
  ...
  if(alerts){
    if(Object.keys(alerts).length == 1) {
      if(alerts.ThermostatsAlert && alerts.ThermostatsAlert.LongText){
        status = "Error";
        if(alerts.ThermostatsAlert.LongText.startsWith("The internet connection to this thermostat was lost")){
          status = "Internet Connection Error";
        }
        status_json = JSON.stringify({"error_message": alerts.ThermostatsAlert.LongText});
      } else{
  ...
  newState[this.status_json_attributes_topic] = status_json;
