...
const minimizeDiscoveryPayload = require('./haDiscovery').minimizeDiscoveryPayload;
...
function MqttThermostat(wrapper, config, device) {
  ...
  this.use_auto_for_schedule_in_homekit = isTrue(config['use_auto_for_schedule_in_homekit'])
  ...
  this.min_temp = getMinMaxTemp(config['min_temp'], Math.min, this.temperatureToMqtt(device.TargetTemperatureHeatMinValue), this.temperatureToMqtt(device.TargetTemperatureCoolMinValue), this.valid_state_modes);
  this.max_temp = getMinMaxTemp(config['max_temp'], Math.min, this.temperatureToMqtt(device.TargetTemperatureHeatMaxValue), this.temperatureToMqtt(device.TargetTemperatureCoolMaxValue), this.valid_state_modes);
  ...
  this.away_temperature = config['away_temperature'] !== undefined
...
MqttThermostat.prototype.generateDiscoveryPayloads = function(minimized=true){
    ...
    if(minimized){
        return [minimizeDiscoveryPayload(payload), minimizeDiscoveryPayload(statusPayload)];
    }
...
MqttThermostat.prototype.onConnect = async function(device) {
  ...
  if (this.homeassistant_discovery) {
    ...
    await this.mqtt.publish("homeassistant/sensor/tcc2mqtt/" + this.ThermostatID + "/config", payloadString, {retain: true});

    await new Promise(r => setTimeout(r, 1000)); // Add sleeps some home assistant can process
  }

  await this.mqtt.publish(this.status_availability_topic, "online", {retain: true});
...
MqttThermostat.prototype.setAvailability = async function(online, force){
  if(this.available !== online || force) {
    ...
    this.log.info("Setting availability to %s", state)
    await this.mqtt.publish(this.availability_topic, state, {retain: true});
  }
...
MqttThermostat.prototype.handleMqttMessage = function(topic, message){
  ...
  switch(topic){
    ...
    case this.hold_command_topic:
      ...
      break;
    default:
      this.log.info("Unhandled message: " + topic + ": " + message.toString())
  }
}
...
MqttThermostat.prototype.updateStatusAsync = async function(device, force = false){
  ...
  this.log.debug("Alerts(%s): %s", alerts === undefined ? "u" : "", JSON.stringify(alerts));
  ...
  if(alerts && Object.keys(alerts).length > 0){
    ...
    if(Object.keys(alerts).length == 1) {
      if(alerts.ThermostatsAlert && alerts.ThermostatsAlert.LongText){
        ...
        status = "Error";
        if(longText.contains("than the alert setting")){
            status = "Temperature Alert";
            available = true;
        }
        if(longText.startsWith("The thermostat did not acknowledge the changes submitted")){
          status = "Change not received";
          available = true;
        }
        if(longText.startsWith("The internet connection to this thermostat was lost")){
...
MqttThermostat.prototype.publishStateAsync = async function(newState, force = false){
