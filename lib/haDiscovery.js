...
const lookup_dict = objectFlip(abbr);
const minimizeDiscoveryPayload = function (input) {
  const ret = {};
  Object.keys(input).forEach((key) => {
    let newKey = key;
    if (key in lookup_dict) {
      newKey = lookup_dict[key];
    }
    ret[newKey] = input[key];
  });
  return ret;
};
module.exports = {minimizeDiscoveryPayload}